...
const DefinitionsContainer = () => {
  const { searchResults, isDefinitionsReady } = useContext<AppContextInterface>(AppContext);
  ...
  if (isDefinitionsReady) {
    if (definitionCardList.length === 0) {
      display = (
        <div className="flex justify-center items-center min-h-full text-lg font-bold text-gray-400 text-center">
          Your search terms did not match any entries.
        </div>
      );
    } else {
      display = definitionCardList;
    }
  } else {
    display = (
      <div className="flex justify-center items-center min-h-full text-lg font-bold text-gray-400 text-center">
        <svg
          className="animate-spin inline mr-2 w-6 h-6 text-gray-200 animate-spin fill-gray-400"
          viewBox="0 0 100 101"
          fill="none"
